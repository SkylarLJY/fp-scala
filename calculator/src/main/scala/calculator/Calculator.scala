 ...
 import Expr.*
  ...
  def computeValues(
      namedExpressions: Map[String, Signal[Expr]]): Map[String, Signal[Double]] =
        namedExpressions.map((str, sig) => (str, Signal{eval(sig(), namedExpressions)})) 

  def eval(expr: Expr, references: Map[String, Signal[Expr]])(using Signal.Caller): Double =
    expr match
      case Literal(v) => v
      case Ref(str) => eval(getReferenceExpr(str, references.removed(str)), references)
        // if references.contains(str) then eval(references(str).currentValue, references) else Double.NaN
