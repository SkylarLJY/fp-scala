...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(arr) => 
        val items = arr.map(decoder.decode(_))
        if !items.filter(_.isEmpty).isEmpty then Option.empty
        else Some(items.map(_.get))
      case _ => Option.empty
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromPartialFunction {
      case Json.Obj(map) if map.contains(name) => decoder.decode(map(name)).get
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name").zip(Decoder.field[Int]("age")).transform[Person](Person(_, _))
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] = 
    ObjectEncoder.field[List[Person]]("people").transform[Contacts](l=>l.people)
