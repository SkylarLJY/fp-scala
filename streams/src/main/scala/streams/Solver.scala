...
trait Solver extends GameDef:
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filterNot(nb=>explored.contains(nb._1))
  ...
  def from(initial: LazyList[(Block, List[Move])],
      ...
      // lazy val allNbs = 
        if initial.isEmpty then initial
        else 
          lazy val newNbs = newNeighborsOnly(initial.flatMap(neighborsWithHistory(_, _)), explored)
          initial ++ from(newNbs, explored ++ newNbs.map(_._1))
