...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw new ju.NoSuchElementException("Empty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = 
    val res = left.filterAcc(p, acc).union(right.filterAcc(p, acc))
    if p(elem) then res.incl(elem)
    else res
  ...
  def union(that: TweetSet): TweetSet = 
    var res = that
    this.foreach(t => res = res.incl(t))
    res

  def mostRetweeted: Tweet = 
    var res = elem
    this.foreach(t => if t.retweets > res.retweets then res = t)
    res
